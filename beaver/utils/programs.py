...
class Unembed(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if self.mask is not None:
            # if the logit is in the position of an UNK or PAD token, set it to an extremely small amount
...
class TransformerProgramBlock(nn.Module):
    ...
    def forward(self, x_cat, x_num, mask=None, **kwargs):
        ...
        # get attention output for types of variables
        if self.n_heads_cat:
        ...
        # returns results of cat and num variables
        return x_cat, x_num
...
class TransformerProgramModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # create a list of transformer blocks, one per layer
        ...
        # dropout randomly makes some elements 0, helps with regularization
        self.dropout = nn.Dropout(dropout)
        ...
        # allows for the user of intermediate functions via hooks
    ...
    def forward(self, x, mask=None, **kwargs):
        # embed categorical input 
        x_cat = self.embed(x)

        # if we are doing positional embedding, concatenate those to the categorical input
        if self.pos_embed is not None:
            x_cat = torch.cat([x_cat, self.pos_embed(x_cat)], -1)

        # repeat with numerical
        x_num = self.num_embed(x)

        # run through the TransformerBlocks
        # note, each block is its own entity
        # one block's output is concatenated to the input it received, and that is the input of the next block
        for block in self.blocks:
            x_cat, x_num = block(x_cat, x_num, mask=mask)

        # if unembed_num is true, we have both numerical and categorical
        # otherwise, we only have categorical
        if self.unembed_num:
        ...
        # if we are pooling the outputs, concatenate a mask and the output together
